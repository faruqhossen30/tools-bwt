    ...
    public function store($page){
        ...
        if ( !empty($pageTrans) ) {
             ...
             return view('frontend.pages', [
                ...
                'recent_posts'  => Page::withTranslation()->translatedIn( app()->getLocale() )->where('type', 'default')->orderByTranslation('id', 'DESC')->get()->toArray(),
                'popular_tools' => Page::withTranslation()->translatedIn( app()->getLocale() )->where('type', 'tool')->where('popular', true)->orderByTranslation('id', 'DESC')->get()->toArray()
            ]);

        } else return redirect()->route('page404');
